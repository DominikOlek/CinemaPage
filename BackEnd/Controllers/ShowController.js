...
const AddInfoOne = async (e) => {
  ...
  let lan = await Language.findOne({
    where: { ID: info.dataValues.LanguageID },
  });
  ...
  e.dataValues.MovieInfoID = info.dataValues.MovieID;
  ...
  let cat = await Category.findOne({
    where: { ID: info.dataValues.CategoryID },
  });
...
const addOne = async (req, res) => {
  try {
    ...
    if (
      ...
      !obj.hasOwnProperty("RoomID") ||
      !obj.hasOwnProperty("NormalPrice") ||
      !obj.hasOwnProperty("VIPPrice") ||
    ...
    for (const e of allInRoom) {
      if (
        (new Date(e.StartTime) < start && new Date(e.EndTime) > start) ||
        (new Date(e.StartTime) < end && new Date(e.EndTime) > end)
      ) {
...
const edit = async (req, res) => {
  try {
    ...
    let order = await Order.findOne({ where: { ShowID: req.params["id"] } });
    if (order) {
        res.status(405).json("Someone is order this show, can't edit");
        return;
    }
    ...
    let end = new Date(show.EndTime);
      if (obj.hasOwnProperty("RoomID")) {
          var room = await Room.findOne({ where: { Number: obj.RoomID } });
          if (!room) {
              return res.status(404).json("No room with this ID");
          }
          let allInRoom = await Show.findAll({
              where: {
                  ...
                  [Op.and]: [
                      Sequelize.where(
                          fn("date", col("StartTime")),
                          "=",
                          show.StartTime.toString().split("T")[0]
                      ),
                  ],
              },
          });
          for (const e of allInRoom) {
              if (
                  ...
                  return res
                      .status(406)
                      .json("Other show is in this room at this time");
              }
          }
          obj.Places = JSON.parse(JSON.stringify(room.Places));
      } else {
          var room = await Room.findOne({ where: { Number: show.RoomID } });
      }
    movie = await MovieInfo.findOne({ where: { ID: movie.MovieID } });
    ...
    if (
      obj.hasOwnProperty("IsIMAX") &&
      obj.IsIMAX &&
      (!movie.IsIMAX || !room.IsIMAX)
    ) {
      return res.status(406).json("This movie or room is not IMAX");
    }
    if (
      obj.hasOwnProperty("IsScreenX") &&
      obj.IsScreenX &&
      (!movie.IsScreenX || !room.IsScreenX)
    ) {
      return res.status(406).json("This movie or room is not ScreenX");
    }
    ...
    if (obj.hasOwnProperty("StartTime") || obj.hasOwnProperty("EndTime")) {
      ...
      for (const e of allInRoom) {
        if (
          (new Date(e.StartTime) < StartA && new Date(e.EndTime) > StartA) ||
          (new Date(e.StartTime) < EndA && new Date(e.EndTime) > EndA)
        ) {
          return res
            .status(406)
            .json("Other show is in this room at this time");
