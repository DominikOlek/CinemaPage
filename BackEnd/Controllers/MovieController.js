...
const AddInfoOne = async (e) => {
    let info = await Movie.findAll({ where: { MovieID: e.ID } });
    if (!info) {
        return;
    }
    e.dataValues.InstID = [];
    e.dataValues.LanguageName = [];
    e.dataValues.SubtitlesName = [];
    for (const el of info) {
        let lan = await Language.findOne({ where: { ID: el.LanguageID } });
        e.dataValues.LanguageName.push(lan.Name);
        lan = await Language.findOne({ where: { ID: el.SubtitlesID } });
        e.dataValues.SubtitlesName.push(lan.Name);       
        e.dataValues.InstID.push(el.ID);
    }
    let cat = await Category.findOne({ where: { ID: e.CategoryID } });
    e.dataValues.Category = cat.Name;
}

const AddInfoOne2 = async (e) => {
...
const getAll = async (req, res) => {
    try {
        let movies = await MovieInfo.findAll();
...
const addOneVersion = async (req, res) => {
    try {
        ...
        let movie = await MovieInfo.findOne({ where: { ID: req.params['id'] } });
